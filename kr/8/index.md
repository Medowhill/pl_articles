...
식이 어떤 정수인 경우, 그 정수가 곧 그 식이 나타내는 값이다. 위 추론 규칙은 가정 없이 결론만 가지고 있다. 추론 규칙은 다음과 같이 해석할 수 있다.
...
\(\{+\ 4\ \{-\ 2\ 1\}\}\)에 적용 가능한 규칙은 하나뿐이다. \(e_1\)을 \(4\)로 \(e_2\)를 \(\{-\ 2\ 1\}\)로 치환하여 가정을 만든다. 단, 값은 비워둔다.
...
\(4\)에 적용 가능한 규칙은 하나뿐이며, 가정이 없는 규칙이다. \(n\)을 \(4\)로 치환하여 값을 얻는다.
...
\(\{-\ 2\ 1\}\)에 적용 가능한 규칙은 하나뿐이다. \(e_1\)을 \(2\)로 \(e_2\)를 \(1\)로 치환하여 가정을 만든다. 단, 값은 비워둔다.
...
\(2\)에 적용 가능한 규칙은 하나뿐이며, 가정이 없는 규칙이다. \(n\)을 \(2\)로 치환하여 값을 얻는다.
...
\(1\)에 적용 가능한 규칙은 하나뿐이며, 가정이 없는 규칙이다. \(n\)을 \(1\)로 치환하여 값을 얻는다.
