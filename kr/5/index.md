...
Scala 표준 라이브러리에는 `List` 클래스가 정의되어 있다. 클래스를 정의한 패키지 이름은 `scala.collection.immutable`이지만, 언제나 들여오기 없이 사용 가능한 [`scala` 패키지](https://www.scala-lang.org/api/current/scala/index.html)에 리스트를 다루기 위한 타입과 값의 *별명*(alias)이 존재하므로 리스트를 사용하기 위해서 들여오기를 할 필요는 없다.
...
쌍반점을 사용한 이전 코드가 더 간결하고 읽기 쉬운 것을 확인할 수 있다.
