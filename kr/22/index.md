이번 글에서는 TFAE에 *매개변수에 의한 다형성*(parametric polymorphism)을 추가하여 TPFAE를 정의한다. TPFAE의 문법, 동적 의미, 타입 체계를 정의하며 타입 검사기와 인터프리터를 구현할 것이다.
...
다형 클래스는 TPFAE가 제공하는 매개변수에 의한 다형성으로 표현되지 않는다. 그럼에도 설명한 이유는 매개변수에 의한 다형성에 대한 직관적 이해를 돕고 사용함으로써 좋은 점을 보이기 위함이다.

## 문법

다음은 TPFAE의 요약 문법이다. TFAE와 비교하여 추가된 부분만 적었다.
...
타입 식별자 \(\alpha\)는 타입으로 사용될 수 있다. 식 \(\lambda\alpha.\lambda x:\alpha.x\)에서 \(\alpha\)가 \(x\)의 타입으로 사용되었다. TVFAE에서 정의한 타입의 이름인 \(t\)를 타입으로 사용할 수 있던 것과 비슷하다. 문법은 타입 식별자가 어디에서 사용되든 상관하지 않는다. 그러나 TVFAE에서 정의하지 않은 타입을 사용할 수 없도록 타입이 올바른 형태인지 검사한 것처럼, TPFAE에서도 타입 식별자가 타입 함수에 묶인 후 올바른 영역에서만 사용되게 해야 한다. 이는 올바른 형태 규칙과 타입 규칙이 할 일로, 뒤에서 볼 것이다.
...
\([\tau/\alpha]\tau'\)은 타입 \(\tau'\)에서 \(\alpha\)를 \(\tau\)로 치환하여 얻은 타입을 나타낸다. 즉, 치환은 타입, 타입 식별자, 타입에서 타입으로 가는 함수이다. \(\alpha\)를 \(\tau\)로 치환한다는 것은 모든 자유 식별자 \(\alpha\)를 \(\tau\)로 대체한다는 뜻이다. \(\tau'\) 안에 \(\alpha\)의 묶는 등장이 있을 수 있다. 그 영역 안에 나오는 \(\alpha\)는 자유 식별자가 아닌 묶인 등장이므로 대체해서는 안 된다.

다음은 타입에 대한 치환을 정의한다.

\[
\begin{array}{rcl}
\lbrack\tau/\alpha\rbrack \textsf{num} &=& \textsf{num} \\
\lbrack\tau/\alpha\rbrack (\tau_1\rightarrow\tau_2) &=&
(\lbrack\tau/\alpha\rbrack \tau_1)\rightarrow(\lbrack\tau/\alpha\rbrack \tau_2) \\
\lbrack\tau/\alpha\rbrack \alpha &=& \tau \\
\lbrack\tau/\alpha\rbrack \alpha' &=& \alpha'\quad
\textsf{(if } \alpha\not=\alpha'\textsf{)} \\
\lbrack\tau/\alpha\rbrack \forall\alpha.\tau' &=& \forall\alpha.\tau' \\
\lbrack\tau/\alpha\rbrack \forall\alpha'.\tau' &=&
\forall\alpha'.\lbrack\tau/\alpha\rbrack \tau'\quad
\textsf{(if } \alpha\not=\alpha'\textsf{)} \\
\end{array}
\]
...
이제 식에 대한 치환을 정의할 수 있다. \([\tau/\alpha]e\)은 식 \(e\)에서 \(\alpha\)를 \(\tau\)로 치환하여 얻은 식을 나타낸다. 이 경우, 치환은 타입, 타입 식별자, 식에서 식으로 가는 함수이다. 타입에 대한 치환과 마찬가지로 \(e\)에서 자유 식별자 \(\alpha\)만 \(\tau\)로 대체해야 한다.

다음은 식에 대한 치환을 정의한다.

\[
\begin{array}{rcl}
\lbrack\tau/\alpha\rbrack n &=& n \\
\lbrack\tau/\alpha\rbrack (e_1+e_2) &=&
\lbrack\tau/\alpha\rbrack e_1+\lbrack\tau/\alpha\rbrack e_2 \\
\lbrack\tau/\alpha\rbrack (e_1-e_2) &=&
\lbrack\tau/\alpha\rbrack e_1-\lbrack\tau/\alpha\rbrack e_2 \\
\lbrack\tau/\alpha\rbrack x &=& x \\
\lbrack\tau/\alpha\rbrack \lambda x:\tau'.e &=&
\lambda x:(\lbrack\tau/\alpha\rbrack\tau').\lbrack\tau/\alpha\rbrack e \\
\lbrack\tau/\alpha\rbrack (e_1\ e_2) &=&
\lbrack\tau/\alpha\rbrack e_1\ \lbrack\tau/\alpha\rbrack e_2 \\
\lbrack\tau/\alpha\rbrack \lambda\alpha.e &=& \lambda\alpha.e \\
\lbrack\tau/\alpha\rbrack \lambda\alpha'.e &=&
\lambda\alpha'.\lbrack\tau/\alpha\rbrack e\quad
\textsf{(if } \alpha\not=\alpha'\textsf{)} \\
\lbrack\tau/\alpha\rbrack (e\ \lbrack\tau'\rbrack) &=&
(\lbrack\tau/\alpha\rbrack e)\ \lbrack\lbrack\tau/\alpha\rbrack \tau'\rbrack \\
\end{array}
\]

역시 대부분의 경우는 이해하기 쉽다. 타입 함수에 대한 치환만 조심하면 된다. 전칭 타입처럼 타입 함수도 타입 식별자를 묶으므로 어떤 타입 식별자의 묶는 등장이 있는지 확인해야 한다.

이제 TPFAE의 동적 의미를 정의하겠다. 추가된 식인 타입 함수와 타입 적용에 대한 추론 규칙만 보겠다. 나머지 규칙은 TFAE의 추론 규칙과 같다.
...
{ \sigma\vdash e\Rightarrow \langle\lambda\alpha.e',\sigma'\rangle \quad
  \sigma'\vdash \lbrack\tau/\alpha\rbrack e'\Rightarrow v }
...
TVFAE에서와 마찬가지로 타입 환경의 정의에 수정이 필요하다. TPFAE의 타입 환경은 변수의 타입을 저장하는 것에 더해 사용할 수 있는 타입 식별자를 저장해야 한다. TVFAE와 달리 타입 식별자에 관련된 정보가 없으므로 묶는 등장을 한 타입 식별자를 저장하기만 하면 된다. 다음은 타입 환경의 정의이다.

\[
\begin{array}{rrcl}
\text{Type Environment} & \Gamma & \in &
\mathit{Id}\cup\mathit{TId}\xrightarrow{\text{fin}}
\text{Type}\cup\{\emptyset\} \\
\end{array}
\]

타입 환경의 공역에 아무런 의미 없는 원소인 공집합을 추가하였다. 편의상 \(\Gamma\lbrack\alpha\rbrack\)라 쓰면 \(\Gamma\lbrack\alpha:\emptyset\rbrack\)을 나타낸 것이라고 정의하겠다.
...
이 규칙을 TPFAE에서 그대로 사용해도 타입 안전성을 해치지 않지만 언어의 표현력을 제한한다. 다음의 식을 생각해 보자.
...
\[
\frac
{ \tau=\lbrack\alpha/\alpha'\rbrack\tau' }
{ \forall\alpha.\tau\equiv\forall\alpha'.\tau' }
\]

같은 타입은 동치인 타입이다. 두 전칭 타입은 묶는 타입 식별자를 같게 만들었을 때 같은 타입이면 동치이다.
...
이제 TPFAE에 추가된 식에 대한 타입 규칙을 보겠다.
...
{ \Gamma\vdash e\ \lbrack\tau\rbrack:\lbrack\tau/\alpha\rbrack\tau' }
\]
...
다음은 TPFAE의 요약 문법을 Scala로 구현한 것이다.
...
case class Fun(x: String, t: TPFAET, b: TPFAE) extends TPFAE
case class App(f: TPFAE, a: TPFAE) extends TPFAE
case class TFun(a: String, b: TPFAE) extends TPFAE
case class TApp(f: TPFAE, t: TPFAET) extends TPFAE

sealed trait TPFAET
case object NumT extends TPFAET
case class ArrowT(p: TPFAET, r: TPFAET) extends TPFAET
case class ForallT(a: String, t: TPFAET) extends TPFAET
case class IdT(a: String) extends TPFAET
```

`TPFAE` 인스턴스는 TPFAE 식을 표현한다. `TFun` 인스턴스는 타입 함수, `TApp`인스턴스는 타입 적용을 나타낸다. `ForallT` 인스턴스는 전칭 타입, `IdT` 인스턴스는 타입으로서의 타입 식별자를 나타낸다. 타입 식별자는 임의의 문자열이다.

```scala
case class TEnv(
  vars: Map[String, TPFAET] = Map(),
  tbinds: Set[String] = Set()
) {
  def +(p: (String, TPFAET)): TEnv =
    copy(vars = vars + p)
  def +(x: String): TEnv =
...
```scala
def subst(t1: TPFAET, a: String, t2: TPFAET): TPFAET = t1 match {
  case NumT => t1
...
```scala
def validType(t: TPFAET, env: TEnv): TPFAET = t match {
  case NumT => t
  case ArrowT(p, r) =>
...
```scala
def typeCheck(e: TPFAE, env: TEnv): TPFAET = e match {
  case Num(n) => NumT
  case Add(l, r) =>
...
이제 TPFAE의 인터프리터를 보겠다.

```scala
sealed trait TPFAEV
case class NumV(n: Int) extends TPFAEV
case class CloV(p: String, b: TPFAE, e: Env) extends TPFAEV
case class TFunV(a: String, b: TPFAE, e: Env) extends TPFAEV
```
...
```scala
def interp(e: TPFAE, env: Env): TPFAEV = e match {
  case Num(n) => NumV(n)
  case Add(l, r) =>
