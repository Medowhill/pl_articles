프로그래밍 언어 과목(이하 CS320) 수강생과 프로그래밍 언어에 관해 더 알기 위해 찾아온 사람들을 환영하며, 모두에게 감사를 표한다. 서문은 두 부분으로 나누어진다. 첫 부분은 CS320 수강생에게 뒤의 글을 읽는 방법을 설명한다. 길지 않으니 수강생이 아니어도 읽기를 추천한다. 두 번째 부분은 내가 글을 쓰기로 한 이유, 글의 방향성, 용어를 한국어화 하는 기준을 정리한다. 개인적인 이야기이므로, 건너뛰어도 문제없이 뒤의 글을 읽을 수 있다.

## 글을 읽는 방법

CS320 자료에 속하는 글은 수업 보조 자료로 작성되었으나, CS320 강의와 일대일 대응을 이루지는 않으므로 읽을 때 주의가 필요하다. 이 글은 서문이므로, 영 번째 글로 생각하고 이야기하겠다.

* ‘프로그래밍 언어’는 프로그래밍 언어라는 전산학의 한 분야가 무엇을 하는지 소개한다. 

첫 글은 첫 수업을 들은 후 바로 읽을 것을 추천한다.

* ‘Scala 소개’는 CS320에서 사용하는 언어인 Scala를 소개한다.

둘째 글은 첫 코딩 과제를 하기 전 읽을 것을 추천한다. 단, Scala를 이미 사용해본 사람은 읽지 않아도 된다.

* ‘함수형 프로그래밍 입문’은 불변성과 재귀, 리스트를 다룬다.
* ‘일급 함수’는 일급 함수와 옵션 타입을 다룬다.
* ‘Scala 컬렉션 사용하기’는 Scala 표준 라이브러리와 `for` 식을 다룬다.
* ‘패턴 대조’는 패턴 대조의 필요성과 다양한 활용을 다룬다.

세 번째 글부터 여섯 번째 글까지 네 개의 글은 함수형 프로그래밍을 소개한다. 함수형 프로그래밍은 CS320의 주제는 아니나, CS320에 나오는 코드가 모두 함수형으로 작성되며, 함수형 프로그래밍을 이해하는 것이 CS320에서 다루는 개념을 이해하는 데 도움이 된다. 네 글을 모두 첫 코딩 과제를 하기 전에 읽을 필요는 없지만, 빨리 읽을수록 강의를 듣고 과제를 하는 데 도움이 되므로, 가능한 한 빨리 읽을 것을 추천한다. 함수형 프로그래밍을 이미 충분히 경험해 본 사람은 읽지 않아도 된다.

* ‘문법’은 언어의 문법이 무엇인지와 문법을 정의하는 방법을 다루고, AE의 문법을 정의한다.
* ‘의미’는 언어의 의미가 무엇인지와 의미를 정의하는 방법을 다루고, AE의 의미를 정의한다.
* ‘식별자’는 식별자를 소개하고 WAE를 정의한다.
* ‘일차 함수’는 일차 함수를 소개하고 F1WAE를 정의한다.
* '클로저‘는 일급 함수와 클로저를 소개하고 FAE를 정의한다.
* ‘람다 대수’는 람다 대수와 Church 인코딩을 소개한다.
* ‘재귀’는 재귀 함수를 소개하고 RCFAE를 정의한다.

일곱 번째 글부터는 CS320 강의 내용과 직접 관련이 있다. 이 글들은 강의를 들은 다음 읽을 것을 전제로 작성하였다. ‘Syntax and Semantics’ 강의 후 ‘문법’과 ‘의미’를 읽을 것을 추천한다. ‘Evaluation of WAE’ 강의 후 ‘식별자’를 읽을 것을 추천한다. ‘First-Order Functions’ 강의 후 ‘일차 함수’를 읽을 것을 추천한다. ‘Functions’ 강의 후 ‘클로저’와 ‘람다 대수’를 읽을 것을 추천한다. ‘Implementing Recursion’ 강의 후 ‘재귀’를 읽을 것을 추천한다.

아직 작성하지 않은 글은 학기 중에 차례대로 추가할 것이며, 서문도 그에 맞춰 수정할 것이다.

CS320 자료에 속하지 않는 글은 프로그래밍 언어 관련 학회에 참석하거나 논문을 읽은 후 작성한 후기이다. CS320을 수강하는 데 도움이 되는 내용은 아니며, 프로그래밍 언어에 관해 더 많이 알고 싶은 사람에게 권한다. 아직 글이 얼마 없는데, 빠르게 추가하기는 어렵겠지만 기회가 될 때 틈틈이 추가하려 한다.

모든 글에는 댓글을 달 수 있으며, 글에 의견이나 질문이 있다면 댓글을 달아주길 부탁한다. 가급적 빠르게 답을 달고 본문에 반영할 예정이다. 댓글 작성 시에 한국어나 영어 중 어느 것을 사용해도 무방하다. 단, 그 밖의 언어는 이해하기 어려우니 지양하길 바란다. 이름은 본명을 사용할 필요는 없지만, 타인을 사칭하면 안 되며, 적어도 한 글 안에서는 이름을 일관적으로 사용하여 의견 교환을 용이하게 하면 좋겠다. 이메일은 입력하지 않아도 되며, 입력하였다면 웹 페이지에는 공개되지 않고 댓글에 답이 달렸음을 알려주는 데만 사용된다. 한 번 작성한 댓글은 수정하거나 지울 수 없으니, 신중하게 작성 바란다.
