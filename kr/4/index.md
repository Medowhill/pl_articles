...
`f`와 `f(0)`를 호출하는 대신 변수가 가리키는 대상으로 지정하는 것도 가능하다. 이 경우 `h0`의 타입은 `Int => (Int => Int)`가 된다. 함수 타입은 일반적으로 *오른쪽* *결합*(right-associative)을 하는 것으로 간주하므로 `Int => Int => Int`라고 표현해도 같은 뜻이다. `h1`은 `f(0)`의 결괏값을 가리키므로 그 타입은 `Int => Int`이다. 함숫값을 가리키는 변수도 `def`를 통해서 선언한 함수 이름과 마찬가지로 함수 호출의 대상이 될 수 있다. 따라서 `h(0)(0)`나 `h1(0)` 모두 가능한 식이며, 둘 모두 `g(0)`와 같으므로 `0`이 결괏값이다.
